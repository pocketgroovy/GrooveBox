...
public class ColdTwit extends Activity {

	private static TextView twitTx,twitTx2,twitTx3,twitTx4,twitTx5;
	private static TextView twitTimeTx,twitTimeTx2,twitTimeTx3,twitTimeTx4,twitTimeTx5;
	private HttpClient client;
	private JSONArray json;
	private JSONObject latest,latest2,latest3,latest4,latest5;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cold_twit);
...
//		new Read(bearerToken).execute("twitter");
	}

    private String getBearerToken()
    {
        String bearerToken = "";
		Log.d("LOG", ">>>>>>>>>>>>>>>>  get bearerToken: ");

        try {
			OAuthHandler authHandler = new OAuthHandler();
            bearerToken = authHandler.getAccessToken("coldplay");
			Log.d("LOG", ">>>>>>>>>>>>>>>>  bearerToken: " + bearerToken);
		}
        catch (Exception e){
	...
	public JSONArray latestTweet(String username, String token) throws ClientProtocolException, IOException, JSONException
	{
...
//			Toast.makeText(ColdTwit.this, ">>>>>>>>>>  Error", Toast.LENGTH_SHORT).show();
...
//				}
//				try {
//					return json = latestTweet("coldplay", _token);
//
